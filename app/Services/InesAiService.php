...
class InesAiService
{
    public function respond(SupportConversation $conversation, string $message): array
    {
        ...
        if ($this->isPriceIntent($normalized)) {
            return $this->result(
                'ინეს ბაღის ყოველთვიური ღირებულება 600 ლარია.',
        ...
        if ($this->isGroupInterestIntent($normalized) || ($context['flow'] ?? null) === 'availability') {
            return $this->result(
                $this->groupAvailabilityMessage(),
                $this->clearFlow($context),
                false,
                'ჯგუფში ადგილი',
            );
    ...
    private function isPriceIntent(string $message): bool
    {
        return $this->matches($message, [
            'ფასი', 'ღირებულება', 'საფასური', 'რა ღირს', 'რამდენი ღირს',
            'რამდენია გადასახადი', 'რამდენია თვეში', 'თვეში რამდენია', 'თვეში რამდენი',
            'თვიური ფასი', 'თვიური გადასახადი', 'გადასახადი რამდენია',
        ]);
    }

    private function isGroupInterestIntent(string $message): bool
    {
        return $this->isAvailabilityIntent($message)
            || $this->matches($message, [
                'ჯგუფი მაინტერესებს', 'ჯგუფში მინდა', 'ჯგუფზე ინფორმაცია',
                'რომელი ჯგუფი', 'ჩარიცხვა', 'რეგისტრაცია', 'განაცხადი',
            ])
            || preg_match('/\\b[2-5]\\s*[-–]\\s*[3-6]\\b/u', $message) === 1;
    }

    private function groupAvailabilityMessage(): string
    {
        return 'ამ ეტაპზე ჯგუფებში ადგილები შევსებულია. ინფორმაციის გადასამოწმებლად დაგვიკავშირდით: 555 411 831.';
    }
    ...
    {
        return $this->matches($message, [
            'ფასდაკლება', 'გადახდა', 'დავალიანება', 'ქვითარი',
...
ყოველთვიური ღირებულება ზუსტად 600 ლარია.
თუ მომხმარებელი რომელიმე ჯგუფით, ჩარიცხვით ან ადგილით ინტერესდება, უპასუხეთ ზუსტად: „ამ ეტაპზე ჯგუფებში ადგილები შევსებულია. ინფორმაციის გადასამოწმებლად დაგვიკავშირდით: 555 411 831.“
არ გამოიგონოთ სხვა ფასი, ადგილი, თარიღი, პოლიტიკა ან ბავშვის პირადი ინფორმაცია.
