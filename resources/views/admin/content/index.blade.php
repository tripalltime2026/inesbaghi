...
<section class="cms-block" id="cms-blog">
    <div class="cms-block-head">
        <div><p class="eyebrow">ბლოგის მართვა</p><h2>სტატიები და ქავერები</h2><p>ატვირთეთ JPG, PNG ან WEBP ქავერი მაქსიმუმ 5 MB-მდე. დრაფტი საჯარო საიტზე არ გამოჩნდება.</p></div>
        <span class="cms-count">{{ $posts->count() }} სტატია</span>
    </div>

    <details class="cms-create-box">
        <summary>+ ახალი სტატიის შექმნა</summary>
        <form method="post" action="{{ route('admin.content.blog.store') }}" enctype="multipart/form-data" class="cms-item-form">
            @csrf
            <div class="cms-field-grid">
                <label class="wide"><span>სტატიის სათაური</span><input name="title" required></label>
                <label><span>კატეგორია</span><input name="category"></label>
                <label><span>სტატუსი</span><select name="status">@foreach($postStatuses as $key => $label)<option value="{{ $key }}">{{ $label }}</option>@endforeach</select></label>
                <label><span>გამოქვეყნების თარიღი</span><input type="datetime-local" name="published_at"></label>
                <label><span>რიგითობა</span><input type="number" name="sort_order" value="0" min="0"></label>
                <label class="wide"><span>მოკლე აღწერა</span><textarea name="excerpt" rows="3"></textarea></label>
                <label class="wide"><span>სტატიის სრული ტექსტი</span><textarea name="body" rows="8"></textarea></label>
                <label><span>ქავერის ატვირთვა</span><input type="file" name="cover" accept="image/jpeg,image/png,image/webp"></label>
                <label><span>ქავერის ALT აღწერა</span><input name="cover_alt"></label>
            </div>
            ...
            <article class="cms-item-card">
                <div class="cms-card-top">
                    <div class="cms-card-identity">
                        ...
                        @endif
                        <div><strong>{{ $post->title }}</strong><small>{{ $post->category ?: 'კატეგორიის გარეშე' }} · {{ $post->published_at?->format('d.m.Y H:i') ?: 'თარიღის გარეშე' }}</small></div>
                    </div>
